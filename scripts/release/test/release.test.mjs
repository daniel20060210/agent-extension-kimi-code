...
test("signs and verifies the actual packaged Kimi Code extension", async () => {
  const root = await temporaryRoot();
...
test("rejects symlinks", async () => {
  ...
  await assert.rejects(validatePackage(packageDir, "kimi-code"), /symlinks/u);
});
...
test("rejects unsafe referenced paths", async () => {
  ...
  await assert.rejects(validatePackage(packageDir, "kimi-code"), /relative package path/u);
});

test("rejects managed executables that escape installRoot", async () => {
  const packageDir = await temporaryFixture();
  await mutateManifest(packageDir, (manifest) => {
    manifest.runtime.launch.executable = "${installRoot}/../outside/kimi-code";
  });
  await assert.rejects(validatePackage(packageDir, "kimi-code"), /stay under/u);
});
...
test("both validators enforce discovery and safe Skill roots", async () => {
  ...
  const composerPath = path.join(packageDir, "profiles", "composer.json");
  const composer = JSON.parse(await readFile(composerPath, "utf8"));
  composer.skills ??= { invocation: "textTrigger", triggerPrefix: "/", roots: [{ scope: "workspace", path: ".agent/skills" }] };
...
test("rejects files not declared by the manifest", async () => {
  ...
  await assert.rejects(validatePackage(packageDir, "kimi-code"), /not declared/u);
});

test("rejects unpinned runtime packages", async () => {
  const packageDir = await temporaryFixture();
  await mutateManifest(packageDir, (manifest) => {
    manifest.runtime.install.args[2] = "kimi-cli";
  });
  await assert.rejects(validatePackage(packageDir, "kimi-code"), /exact package==version/u);
});
...
test("rejects oversized presentation assets", async () => {
  ...
  await assert.rejects(validatePackage(packageDir, "kimi-code"), /exceeds 256 KiB/u);
});
...
test("rejects active or remotely loaded SVG assets", async () => {
  ...
  await assert.rejects(validatePackage(packageDir, "kimi-code"), /active or remote/u);
});
    ...
    `${JSON.stringify(
      {
        ...
        runtime: {
          ...
          install: {
            runner: "uv",
            args: ["tool", "install", "kimi-cli==1.49.0"]
          },
          launch: {
            executable: "${installRoot}/bin/kimi",
            args: ["acp"]
          }
        },
    ...
    `${JSON.stringify(
      {
        ...
        candidates: [
          {
            binaryNames: ["kimi"],
            version: { args: ["--version"], constraint: ">=1.49.0 <2.0.0" },
            launchArgs: ["acp"],
